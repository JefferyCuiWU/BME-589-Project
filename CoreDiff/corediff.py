...
class corediff(TrainTask):
    ...
    def test(self, n_iter):
        ...
        for low_dose, full_dose in tqdm.tqdm(self.test_loader, desc='test'):
            low_dose, full_dose = low_dose.cuda(), full_dose.cuda()
            full_dose = full_dose[:, 0:1, :, :]  # [B, 1, H, W]
    ...
    def generate_images(self, n_iter):
        ...
        output_dir = os.path.join("results", f"{opt.run_name}_npy")
        os.makedirs(output_dir, exist_ok=True)

        idx_counter = 0  # global slice counter

        for low_dose, full_dose in self.test_loader:
            low_dose, full_dose = low_dose.cuda(), full_dose.cuda()

            # Use only center slice if triplet
            if low_dose.shape[1] == 3:
                low_dose_center = low_dose[:, 1:2]
            else:
                low_dose_center = low_dose
            ...
            # Also convert gen_full_dose to center if needed
            if gen_full_dose.shape[1] == 3:
                gen_full_dose = gen_full_dose[:, 1:2]

            # Save each output slice as one .npy
            for i in range(gen_full_dose.shape[0]):
                slice_numpy = gen_full_dose[i, 0].cpu().numpy()  # shape: (H, W)
                 # Check for invalid values
                if not np.isfinite(slice_numpy).all():
                    print(f"[WARNING] Slice {idx_counter} contains NaNs or Infs. Skipping.")
                    continue
                save_path = os.path.join(output_dir, f"ct_{idx_counter:04d}.npy")
                ...
                try:
                    np.save(save_path, slice_numpy)
                except Exception as e:
                    print(f"[ERROR] Failed to save {save_path}: {e}")
                    continue
                idx_counter += 1

        print(f"Saved {idx_counter} predicted slices to {output_dir}")
    ...
    def test_osl_framework(self, test_iter):
        ...
        for low_dose, full_dose in tqdm.tqdm(self.test_loader, desc='test'):
            low_dose, full_dose = low_dose.cuda(), full_dose.cuda()
            print("low_dose shape:", low_dose.shape)
            print("full_dose shape:", full_dose.shape)
            gen_full_dose, direct_recons, imstep_imgs = self.ema_model.sample(
                batch_size=low_dose.shape[0],
                img=low_dose,
                t=self.T,
